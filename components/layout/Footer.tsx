import Link from 'next/link'
import { Mail, Phone, MapPin, Share2, Globe, Video } from 'lucide-react'
import Logo from '@/components/Logo'

export default function Footer() {
  return (
    <footer className="bg-navy-950 border-t border-gold-500/10 pt-16 pb-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 mb-12">
          {/* Brand */}
          <div className="lg:col-span-1">
            <Logo size={72} className="mb-4" />
            <p className="text-white/50 text-sm leading-relaxed">
              A mais tradicional competição de futsal do Sudoeste do Paraná, organizada pelo
              Grêmio Industrial Patobranquense desde 1996.
            </p>
            <div className="flex gap-3 mt-5">
              {[
                { Icon: Share2, href: '#' },
                { Icon: Globe, href: '#' },
                { Icon: Video, href: '#' },
              ].map(({ Icon, href }, i) => (
                <a key={i} href={href} className="w-9 h-9 rounded-lg bg-white/5 flex items-center justify-center text-white/50 hover:text-gold-400 hover:bg-gold-500/10 transition-all">
                  <Icon size={16} />
                </a>
              ))}
            </div>
          </div>

          {/* Links */}
          <div>
            <h4 className="text-gold-400 font-semibold text-sm uppercase tracking-wider mb-4">Competição</h4>
            <ul className="space-y-2">
              {[
                { href: '/competicao', label: 'Sobre o Campeonato' },
                { href: '/competicao#regulamento', label: 'Regulamento' },
                { href: '/competicao#categorias', label: 'Categorias' },
                { href: '/competicao#calendario', label: 'Calendário' },
                { href: '/competicao#premiacao', label: 'Premiação' },
              ].map(link => (
                <li key={link.href}>
                  <Link href={link.href} className="text-white/50 text-sm hover:text-gold-400 transition-colors">
                    {link.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          <div>
            <h4 className="text-gold-400 font-semibold text-sm uppercase tracking-wider mb-4">Navegação</h4>
            <ul className="space-y-2">
              {[
                { href: '/jogos', label: 'Jogos e Resultados' },
                { href: '/classificacao', label: 'Classificação' },
                { href: '/galeria', label: 'Galeria de Fotos' },
                { href: '/inscricao', label: 'Fazer Inscrição' },
              ].map(link => (
                <li key={link.href}>
                  <Link href={link.href} className="text-white/50 text-sm hover:text-gold-400 transition-colors">
                    {link.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Contact */}
          <div>
            <h4 className="text-gold-400 font-semibold text-sm uppercase tracking-wider mb-4">Contato</h4>
            <ul className="space-y-3">
              <li className="flex items-start gap-2 text-white/50 text-sm">
                <MapPin size={15} className="text-gold-500 flex-shrink-0 mt-0.5" />
                <span>R. Araucária, 883 - Santa Terezinha<br />Pato Branco – PR, 85501-160</span>
              </li>
              <li className="flex items-center gap-2 text-white/50 text-sm">
                <Phone size={15} className="text-gold-500 flex-shrink-0" />
                <a href="tel:+5546984136469" className="hover:text-gold-400 transition-colors">(46) 98413-6469</a>
              </li>
              <li className="flex items-center gap-2 text-white/50 text-sm">
                <Mail size={15} className="text-gold-500 flex-shrink-0" />
                <a href="mailto:contato@luigipatriarcha.com.br" className="hover:text-gold-400 transition-colors">
                  contato@luigipatriarcha.com.br
                </a>
              </li>
            </ul>
            <a
              href="https://wa.me/5546984136469"
              target="_blank"
              rel="noreferrer"
              className="mt-4 inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-green-600/20 text-green-400 text-sm font-medium hover:bg-green-600/30 transition-all border border-green-600/20"
            >
              <svg viewBox="0 0 24 24" className="w-4 h-4 fill-current"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
              WhatsApp
            </a>
          </div>
        </div>

        {/* Bottom */}
        <div className="pt-8 border-t border-gold-500/10 flex flex-col sm:flex-row justify-between items-center gap-3">
          <p className="text-white/30 text-xs">
            © 2026 29ª Taça Luigi Patriarcha de Futsal · Grêmio Industrial Patobranquense. Todos os direitos reservados.
          </p>
          <div className="flex gap-4 text-white/30 text-xs">
            <Link href="/privacidade" className="hover:text-gold-400 transition-colors">Privacidade</Link>
            <Link href="/termos" className="hover:text-gold-400 transition-colors">Termos de Uso</Link>
          </div>
        </div>
      </div>
    </footer>
  )
}
